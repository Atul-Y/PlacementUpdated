...
int minCostRopes(vector<int> &arr)
{
    ...
    int totalCost = 0;
    while (mnhp.size() > 1) // we will keep connecting till only one rope is left
    {
        int first = mnhp.top();
        mnhp.pop();
        int second = mnhp.top();
        mnhp.pop();
        int cost = first + second; // cost to connect two ropes
        totalCost += cost;         // add this cost to total cost
        mnhp.push(cost);           // push the new rope back to heap
    }
    return totalCost;
}
int main()
{
    ...
    cout << "Minimum cost to connect ropes: " << minCostRopes(arr) << endl; // Output: 29
    return 0;
}